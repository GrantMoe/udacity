...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if (beta >= 0.001):
            ...
            # turning circle center coordinates:
            #   cx = x - sin(theta) * R
            #   cy = y + cos(theta) * R
            cx = x - sin(theta) * R
            cy = y + cos(theta) * R
            ...
            #   theta' = (theta + beta) mod 2pi
            x_prime = cx + sin(theta + beta) * R
            y_prime = cy - cos(theta + beta) * R
            theta_prime = (theta + beta) % (2.0 * pi)

        else: 
        # small beta (< 0.001):
        #   x' = x + d*cos(theta)
        #   y' = y + d*sin(theta)
        #   theta' = theta (technically theta + beta) mod 2pi
            theta_prime = (theta + beta) % (2.0 * pi)
            x_prime = x + (d * cos(theta_prime))
            y_prime = y + (d * sin(theta_prime))
